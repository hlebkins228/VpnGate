...
| `wscat` сразу получает `error: Unexpected server response: 400`, в логах Yandex Cloud `code: 400, event_type: CONNECT, response_length_bytes: 29` | Это «`missing connection id header`» от нашего сервера. У `type: http`-интеграции Yandex API Gateway **по умолчанию не форвардит заголовки оригинального запроса** на бэкенд — заголовки `X-Yc-Apigateway-Websocket-*` (включая connection-id) до VPN-сервера не доходят. Лечится строчкой `headers: { '*': '*' }` в каждой websocket-интеграции — она есть в актуальном `examples/api-gateway.yaml`. После правки спеки: `yc serverless api-gateway update --id <ID> --spec api-gateway.yaml`. |
| В логе сервера `connID` = литерал `{X-Yc-Apigateway-Websocket-Connection-Id}` (со скобками) | В спеке использовался substitution `{name}` без `'*': '*'`. Substitution в `type: http`-интеграции у Yandex работает **только если параметр объявлен в `parameters:`-блоке операции**; для websocket-заголовков проще использовать `headers: { '*': '*' }`, как в актуальном `examples/api-gateway.yaml`. |
