...
| `wscat` сразу получает `error: Unexpected server response: 400`, в логах Yandex Cloud `code: 400, event_type: CONNECT, response_length_bytes: 29` | Это «`missing connection id header`» от нашего сервера. У `type: http`-интеграции Yandex API Gateway по умолчанию **не** форвардит заголовки `X-Yc-Apigateway-Websocket-*`; их нужно явно прописать в `headers:` каждой интеграции через substitution `{X-Yc-Apigateway-Websocket-Connection-Id}` и т. д. Используйте `examples/api-gateway.yaml` как есть и не вырезайте оттуда блоки `headers:`. После правки спеки выполните `yc serverless api-gateway update --id <ID> --spec api-gateway.yaml`. |
