//go:build windows

// Бинарник myvpn-client-windows — VPN-клиент для Windows 11.
//
// Отдельный entrypoint от cmd/client (Linux), потому что под Windows нет
// /dev/net/tun: нужен Wintun-драйвер, NetSh и иные команды управления
// сетью. Шифрование, сжатие и WebSocket-транспорт переиспользуются как есть.
//
// Перед запуском:
//
//  1. Скачать `wintun.dll` нужной разрядности с https://www.wintun.net/ и
//     положить рядом с этим .exe (или в %SystemRoot%\System32).
//  2. Запустить от имени администратора (требуется для создания сетевого
//     адаптера и правки таблицы маршрутизации).
//
// Параметры совпадают с Linux-клиентом: либо флаги, либо переменные
// окружения с теми же именами (MYVPN_SERVER, MYVPN_KEY, MYVPN_CLIENT_IP,
// MYVPN_VERBOSE и т.п.).
package main

import (
	"encoding/hex"
	"flag"
	"log"
	"net/http"
	_ "net/http/pprof"
	"os"
	"os/signal"
	"strings"
	"syscall"

	"myvpn/client/winclient"
	"myvpn/internal/envcfg"
)

func main() {
	var (
		serverURL = flag.String("server", envcfg.String("MYVPN_SERVER", ""),
			"VPN server WebSocket URL (e.g. wss://...apigw.yandexcloud.net/ws). Env: MYVPN_SERVER.")
		keyFile = flag.String("key", envcfg.String("MYVPN_KEY", ""),
			"Path to encryption key file (32 bytes binary or 64 hex chars). Env: MYVPN_KEY.")
		clientIP = flag.String("ip", envcfg.String("MYVPN_CLIENT_IP", "10.0.0.2"),
			"Client IP address inside the tunnel. Env: MYVPN_CLIENT_IP.")
		verbose = flag.Bool("verbose", envcfg.Bool("MYVPN_VERBOSE", false),
			"Enable verbose logging (logs every packet). Env: MYVPN_VERBOSE.")
	)
	flag.Parse()

	tunGateway := envcfg.String("MYVPN_TUN_GATEWAY", "10.0.0.1")
	autoRoutes := envcfg.Bool("MYVPN_AUTO_ROUTES", true)
	insecureTLS := envcfg.Bool("MYVPN_INSECURE_TLS", false)
	pprofAddr := envcfg.String("MYVPN_PPROF_ADDR", "")
	wsHeadersRaw := envcfg.String("MYVPN_WS_HEADERS", "")

	if *serverURL == "" {
		log.Fatal("Server URL is required. Pass -server wss://... or set MYVPN_SERVER env var.")
	}
	if *keyFile == "" {
		log.Fatal("Key file is required. Pass -key C:\\path\\to\\key or set MYVPN_KEY env var.")
	}

	keyData, err := os.ReadFile(*keyFile)
	if err != nil {
		log.Fatalf("Failed to read key file: %v", err)
	}

	const keySize = 32
	const hexKeySize = 64

	var key []byte
	switch len(keyData) {
	case hexKeySize:
		key, err = hex.DecodeString(string(keyData))
		if err != nil {
			log.Fatalf("Failed to decode hex key: %v", err)
		}
		log.Println("Key file detected as hex format, converted to binary")
	case keySize:
		key = keyData
	default:
		log.Fatalf("Invalid key size: expected %d bytes (binary) or %d chars (hex), got %d",
			keySize, hexKeySize, len(keyData))
	}

	headers, err := parseHeaders(wsHeadersRaw)
	if err != nil {
		log.Fatalf("Failed to parse MYVPN_WS_HEADERS: %v", err)
	}

	vpn, err := winclient.NewVPNClient(winclient.VPNClientConfig{
		ServerURL:          *serverURL,
		Key:                key,
		ClientIP:           *clientIP,
		TunGateway:         tunGateway,
		Verbose:            *verbose,
		AutoRoutes:         autoRoutes,
		ExtraHeaders:       headers,
		InsecureSkipVerify: insecureTLS,
	})
	if err != nil {
		log.Fatalf("Failed to create VPN client: %v", err)
	}

	sigChan := make(chan os.Signal, 1)
	signal.Notify(sigChan, os.Interrupt, syscall.SIGTERM)

	if pprofAddr != "" {
		go func() {
			log.Printf("Starting pprof server on %s", pprofAddr)
			log.Println(http.ListenAndServe(pprofAddr, nil))
		}()
	}

	errChan := make(chan error, 1)
	go func() {
		if err := vpn.Connect(); err != nil {
			errChan <- err
		}
	}()

	log.Println("VPN client started. Press Ctrl+C to stop.")

	select {
	case <-sigChan:
		log.Println("Shutting down client...")
	case err := <-errChan:
		log.Printf("Connection error: %v", err)
	}

	if err := vpn.Close(); err != nil {
		log.Printf("Error closing client: %v", err)
	}
	log.Println("Client stopped.")
}

// parseHeaders разбирает строку "Key1: Val1, Key2: Val2" в http.Header.
func parseHeaders(raw string) (http.Header, error) {
	if strings.TrimSpace(raw) == "" {
		return nil, nil
	}
	headers := http.Header{}
	for _, item := range strings.Split(raw, ",") {
		item = strings.TrimSpace(item)
		if item == "" {
			continue
		}
		idx := strings.Index(item, ":")
		if idx <= 0 {
			return nil, &headerParseError{Raw: item}
		}
		k := strings.TrimSpace(item[:idx])
		v := strings.TrimSpace(item[idx+1:])
		if k == "" {
			return nil, &headerParseError{Raw: item}
		}
		headers.Add(k, v)
	}
	return headers, nil
}

type headerParseError struct {
	Raw string
}

func (e *headerParseError) Error() string {
	return "expected 'Key: Value' header, got: " + e.Raw
}
